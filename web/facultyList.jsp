...
<form method="get" action="facultyList.jsp">
...
if(fid != null && !fid.trim().isEmpty()){
    sql.append(" AND F.F_ID = ?");
   params.add(String.valueOf(Integer.parseInt(fid.trim())));
}

// SEARCH BY FULL NAME
if(name != null && !name.trim().isEmpty()){
    sql.append(" AND (UPPER(F.FIRST_NAME) LIKE ? OR UPPER(F.LAST_NAME) LIKE ?)");
...
<a href="editFaculty.jsp?fid=<%=rs.getInt("F_ID")%>"
class="edit-btn">
Edit
</a>
...
"facultyList.jsp?title="+encodeURIComponent(title)+
"&filterName="+encodeURIComponent(fname)+
...
"&disability="+encodeURIComponent(disability)+
